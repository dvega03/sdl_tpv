...
void Texture::loadFromText(SDL_Renderer* renderer, const char* text, const Font& font, SDL_Color color)
{
	SDL_Surface* textSurface = font.generateSurface(text, color);
	if (textSurface == nullptr) throw exception("Unable to render text surface.");
	else
	{
		...
		if (texture == nullptr)
		{
			throw exception("Unable to create texture form text.");
			w = h = 0;
		}
		else
		{
